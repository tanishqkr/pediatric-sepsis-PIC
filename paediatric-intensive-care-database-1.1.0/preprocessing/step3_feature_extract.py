...
Outputs
-------
  output2/cohort_step3_features.csv   — full feature matrix, no imputation yet
  output2/logs/step3_log.txt          — feature counts, missing rates
"""
...
OUT_CSV     = os.path.join(OUTPUT_DIR, "cohort_step3_features.csv")
WINDOW_HOURS = 24
...
def load_labevents(data_dir, subjects, caps):
    ...
    with open(os.path.join(data_dir, 'LABEVENTS.csv'), encoding='utf-8') as f:
        for row in csv.DictReader(f):
            ...
            if mapping['convert']:
                raw = mapping['convert'](raw)

            # Apply physiological cap (on converted value)
            if cap_info:
                # For converted values, we check against cap directly
                # convert_factor in caps is for raw values;
                # here conversion already applied, so use factor=1
                adjusted_cap = {
                    'floor': cap_info['floor'],
                    'cap':   cap_info['cap'],
                    'convert_factor': 1.0,
                }
                val = apply_cap(raw, adjusted_cap)
            else:
                val = raw
...
def run_step3(data_dir, output_dir, log_dir):
    ...
    log_path = os.path.join(log_dir, 'step3_log.txt')
    with open(log_path, 'w') as f:
        f.write(log_str)
    print(f"\nLog saved -> {log_path}")
    print("\nStep 3 complete. Ready for Step 4 (Phoenix labeling).")
