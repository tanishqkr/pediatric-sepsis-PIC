...
Outputs
-------
  output2/cohort_step3.2_features.csv   — full feature matrix, no imputation yet
  output2/logs/step3.2_log.txt          — feature counts, missing rates
"""
...
OUT_CSV     = os.path.join(OUTPUT_DIR, "cohort_step3.2_features.csv")
WINDOW_HOURS = 24
...
LAB_FEATURES = {
    ...
    'hemoglobin':       {'itemids': ['5099','5257'],            'unit': 'g/dL',
                         ...
                         'cap_after_convert': True},  # cap is in g/dL not g/L
...
def load_labevents(data_dir, subjects, caps):
    ...
    for feat, info in LAB_FEATURES.items():
        ...
        for iid in info['itemids']:
            itemid_map[iid] = {
                ...
                'cap_after_convert': info.get('cap_after_convert', False),
            }
    ...
    with open(os.path.join(data_dir, 'LABEVENTS.csv'), encoding='utf-8') as f:
        for row in csv.DictReader(f):
            ...
            # Cap check order depends on whether cap is defined in raw or converted units.
            #
            # cap_after_convert=False (default): cap is in RAW units
            #   e.g. glucose cap=80 mmol/L, raw is mmol/L -> check raw, then convert to mg/dL
            #   e.g. creatinine cap=2000 umol/L, raw is umol/L -> check raw, then convert to mg/dL
            #
            # cap_after_convert=True: cap is in CONVERTED units
            #   e.g. haemoglobin cap=25 g/dL, raw is g/L -> convert to g/dL first, then check cap
            #
            cap_after = mapping.get('cap_after_convert', False)

            if cap_after:
                # Convert first, then check cap
                converted = mapping['convert'](raw) if mapping.get('convert') else raw
                if cap_info:
                    raw_cap = {'floor': cap_info['floor'], 'cap': cap_info['cap'],
                               'convert_factor': 1.0}
                    val = apply_cap(converted, raw_cap)
                else:
                    val = converted
            else:
                # Check cap on raw value first, then convert
                if cap_info:
                    raw_cap = {'floor': cap_info['floor'], 'cap': cap_info['cap'],
                               'convert_factor': 1.0}
                ...
                val = mapping['convert'](raw_checked) if mapping.get('convert') else raw_checked
...
def run_step3(data_dir, output_dir, log_dir):
    ...
    log_path = os.path.join(log_dir, 'step3.2_log.txt')
    with open(log_path, 'w') as f:
        f.write(log_str)
    print(f"\nLog saved -> {log_path}")
    print("\nStep 3.2 complete. All cap/conversion bugs fixed. Ready for Step 4.")
